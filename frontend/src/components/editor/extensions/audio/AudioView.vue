<template>
  <InlineBlockBox>
    <div
      class="relative block max-w-full overflow-hidden rounded-md transition-all"
      :class="{
    ...
    >
      <!-- 初始化状态 -->
      <div v-if="initialization" class="audio-placeholder">
        <div class="placeholder-content">
          <el-icon class="placeholder-icon"><Microphone /></el-icon>
          <p>点击添加音频</p>
          <el-button type="primary" @click="handleAddAudio">
            添加音频
          </el-button>
        </div>
      </div>
      ...
      <div v-else-if="src" class="group relative">
        ...
        <!-- 悬停工具栏 -->
        <div
          class="absolute right-2 top-2 hidden group-hover:flex space-x-2"
        >
...
import { Microphone, Refresh, Delete } from '@element-plus/icons-vue'
...
const containerStyle = computed(() => {
  ...
  style.width = initialization.value ? '100%' : 'fit-content'
  return style
})
...
<style scoped>
.audio-placeholder {
  @apply border-2 border-dashed border-gray-300 rounded-lg p-5 text-center;
  min-height: 110px;
  display: flex;
  align-items: center;
